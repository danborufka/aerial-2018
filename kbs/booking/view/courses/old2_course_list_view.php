				...
				<div style="display: block">
					<? if($_SESSION["user_is_organizer"] == 1 || $_SESSION["user_is_admin"] == 1) { ?>
						...
						<div id="button-open-tasks" class="rb-button3">
							<span class="blink">Offene Aufgaben (<?=$db_functions->courses->db_count_todo()?>)</span>
						</div>
					<? } ?>
				</div>
			</div>
				...
				<form name="form1" id="form1" method="POST" action="process.php">
					...
					<div id="search-option-row1">
						...
						<select name='filter_status' id='filter_status' class='select-with-symbols'>
							...
							<option value='-1'<?php echo ($_SESSION["filter_status"] == -1)? " selected":"" ?>>✎&nbsp&nbsp&nbsp/&nbsp&nbsp&nbsp✔&nbsp&nbsp&nbsp/&nbsp&nbsp&nbsp✔✔ </option>";
							<option value='1'<?php echo ($_SESSION["filter_status"] == 1)? " selected":"" ?>>✎   in Bearbeitung</option>
							<option value='2'<?php echo ($_SESSION["filter_status"] == 2)? " selected":"" ?>>✔   aktiv</option>
							<option value='3'<?php echo ($_SESSION["filter_status"] == 3)? " selected":"" ?>>✔✔  bezahlt (Bezahlung vollständig)</option>
							<option value='4'<?php echo ($_SESSION["filter_status"] == 4)? " selected":"" ?>>✔✔✔ erledigt (bezahlt, Trainer entlohnt)</option>
							<option value='0'<?php echo ($_SESSION["filter_status"] == 0)? " selected":"" ?>>✖   deaktiviert</option>
							<option value='-2'<?php echo ($_SESSION["filter_status"] == -2)? " selected":"" ?>>Ⓐ alle</option>
						</select>
						<label for='filter_course_number'>Kurs- Nr.:</label>
						<input style= "margin-left: 2px; margin-right: 19px;" type="text" name="filter_course_number" id="filter_course_number" value="<?=$_SESSION["filter_course_number"]?>">
					</div>
					<div id="search-option-row2">
						<label for='zeitraum'>Zeitraum:</label>
						 <select name='filter_zeitraum' id='filter_zeitraum'>
							<option value='aktuell'<?php echo ($_SESSION["filter_zeitraum"] == "aktuell")? " selected":"" ?>>aktuell und zukünftig</option>
							<option value='ab_heute'<?php echo ($_SESSION["filter_zeitraum"] == "ab_heute")? " selected":"" ?>>ab heute</option>
							<option value='von_bis'<?php echo ($_SESSION["filter_zeitraum"] == "von_bis")? " selected":"" ?>>von bis</option>
							<option value='all'<?php echo ($_SESSION["filter_zeitraum"] == "all")? " selected":"" ?>>uneingeschränkt</option>
						</select>
						<label for="begin_date1" id="search-option-row2-from">von:</label>
						<input name="filter_from_date" type="text" id="begin_date1" value="<?php echo $_SESSION["filter_from_date"] ?>" readonly>
						<label for="end_date1" id="search-option-row2-to">bis:</label>
						<input name="filter_to_date" type="text" id="end_date1" value="<?php echo $_SESSION["filter_to_date"] ?>" readonly>
							
						<label for='filter_student_email'>Teilnehmer:</label>
						<input style= "margin-left: 2px; margin-right: 19px;" type="text" name="filter_student_email" id="filter_student_email" placeholder="E-Mail-Adresse" value="<?=$_SESSION["filter_student_email"]?>">
						
					</div>
					<div id="search-option-row3">
						<label for='filter_categories'>Kategorie:</label>
						<select name='filter_categories' id='filter_categories'>
							<?php $db_functions->select_options->db_get_category_select_options($_SESSION["filter_categories"], true); ?>
						</select>
						<label for='filter_subcategories'>Unterkat.:</label>
						<select name='filter_subcategories' id='filter_subcategories'>
							<?php $db_functions->select_options->db_get_subcategory_select_options($_SESSION["filter_subcategories"], true, false); ?>
						</select>
						<label for='filter_publishing'>Veröffentl.:</label>
						<select name='filter_publishing' id='filter_publishing'>
									<option value='-1'<?php echo ($_SESSION["filter_publishing"] == -1)? " selected":"" ?>>alle</option>
									<option value='1'<?php echo ($_SESSION["filter_publishing"] == 1)? " selected":"" ?>>✎ bereit zur Veröffentlichung</option>
									<option value='2'<?php echo ($_SESSION["filter_publishing"] == 2)? " selected":"" ?>>✔ veröffentlicht solange Status aktiv</option>
									<option value='3'<?php echo ($_SESSION["filter_publishing"] == 3)? " selected":"" ?>>✖ nicht veröffentlicht</option>
						</select>
						<label for="filter_list_view_mode">Ansicht:</label>
						<select name='filter_list_view_mode' id='filter_list_view_mode' class='select-with-symbols'>
							<option value='1'<?php echo ($_SESSION["filter_list_view_mode"] == 1)? " selected":"" ?>>Listenansicht </option>";
							<option value='2'<?php echo ($_SESSION["filter_list_view_mode"] == 2)? " selected":"" ?>>Blockansicht</option>
						</select>
						<div id="button-reset-options" id="clear-button" class="rb-button" style="width: 210px">
							Optionen zurücksetzen
						</div>
					</div>
					<div id="search-option-row4">
						...
						<button type="submit" id="search-button" class="rb-button" >Ansichtsoptionen anwenden</button>
					</div>
				</form>
...
<div style="display: none;" id="dialog-publishing" title="Veröffentlichen?">
  <p>Sollen alle Kurse, deren Statusse aktiv sind und die bereit zur Veröffentlichung sind, jetzt veröffentlicht werden?</p>
</div>
