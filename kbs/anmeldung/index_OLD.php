...
if (!(		(isset($_GET["confirm"]) || isset($_GET["unsubscribe"]))
		...
		if($confirm_result["result"] == true) {
			if($confirm_result["status"] == 2) {
				...
				if(isset($confirm_result["only_cash_allowed"]) && $confirm_result["only_cash_allowed"]) {
					...
					$url = "http://aerialsilk.at/kurse/#confirmation-ca";
					header('Location: ' . $url);
					exit(); 
				}else {
					$mail_functions->send_system_mail($confirm_result["registration_id"], "regular_payment");
					$url = "http://aerialsilk.at/kurse/#confirmation";
					header('Location: ' . $url);
					exit();  
				}
				...
				$url = "http://aerialsilk.at/kurse/#confirmation-waitlist";
				header('Location: ' . $url);
				exit();
			}
		}else {
			if(isset($confirm_result["already_used"]) && $confirm_result["already_used"]) {
				...
				$url = "http://aerialsilk.at/kurse/#confirmation-already-done";
				header('Location: ' . $url);
				exit();
			}
		}
	}else {
		$url = "http://aerialsilk.at/kurse/#confirmation-failed";
		header('Location: ' . $url);
		exit();
		$result_message = "Bestätigung konnte nicht durchgeführt werden.";
	}

	$url = "http://aerialsilk.at/kurse/#confirmation-invalid";
	header('Location: ' . $url);
	exit();
	$result_message = "Bestätigung konnte nicht durchgeführt werden, bitte melde dich erneut zum Kurs an.";
}
	...
	<div id = "registration-box-layer-behind">
		<div id = "registration-box-layer-top">
			<div id="course-description">
				...
				?>
				<? if ($show_course_data) {
						if($kursdaten['textblock_mode'] != 1 || empty($kursdaten['textblock'])) { ?>
					<ul>
						<li><?=$kursdaten['kursname']?></li>
						<li><? echo (($kursdaten['one_date_only'] == 1 ) ? "Datum" : "Beginn");  ?>: <?=$kursdaten['begin']?></li>
					...
					<? if(!empty($kursdaten['precondition'])) { ?>
			...
			<? if ($show_form)
			{
				...
				<form action="?kurs=<?=$_GET["kurs"]?>&code=<?=$_GET["code"]?>" name="registration-form" method="post" >
				  <? if(isset($kursdaten['is_kid_course']) && $kursdaten['is_kid_course'] == 1) { ?>
						<input type="text" name="kid_name" id ="kid_name" class="rb-input" placeholder="Vorname des Kindes"  value=<?=$_POST["kid_name"]?> >
					...
					<input type="text" name="prename" id ="prename" class="rb-input" placeholder="<?=$placeholder_vorname?>" required value=<?=$_POST["prename"]?> >
					<input type="text" name="surname" id ="surname" class="rb-input" placeholder="<?=$placeholder_nachname?>"  required value=<?=$_POST["surname"]?> >
					<input type="email" name="email" id ="email" class="rb-input" placeholder="Email" required value=<?=$_POST["email"]?> >
					...
					<div id="newsletter-row">
						...
						<div style="display:block">
							...
							<div class="checkbox-text"><a href="http://aerialsilks.at/index.php?lang=de&fetch=page3_4" target="_blank">AGB</a> gelesen und akzeptiert</div>
						</div>
					</div>
					<div class="g-recaptcha" data-sitekey="6LcV7Q4TAAAAAM1mv4VoUOee1Xb05RXI2_KXVB05"></div>
					
					<input type="submit" name="anmeldung_submit" value="<? echo (isset($kursdaten['ausgebucht']) && $kursdaten['ausgebucht'] == 'AUSGEBUCHT') ? 'für Warteliste anmelden' : 'Anmelden'?>">
				</form>
			<?
			} ?>
		</div>
